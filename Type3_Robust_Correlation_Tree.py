# -*- coding: utf-8 -*-
"""
Created on Sun Aug  2 14:58:38 2020
...
def Find_split_var_by_partialCor(df_original, dataSet):
    ...
    for featIndex in range(2,num_feature):
        ranked_y = pd.Series.to_frame(df3.iloc[:,0].rank(ascending=1))
        ranked_x = pd.Series.to_frame(df3.iloc[:,1].rank(ascending=1))
        ranked_z = pd.Series.to_frame(df3.iloc[:,featIndex].rank(ascending=1))
      
        lm = lr.LinearRegression()
        model_zy = lm.fit(ranked_z, ranked_y)
        lm = lr.LinearRegression() ### has to be twice ##
        model_zx = lm.fit(ranked_z, ranked_x)
        
        residual_zy = ranked_y - model_zy.predict(ranked_z)
        residual_zx = ranked_x - model_zx.predict(ranked_z)
    ...
    if df_original[df_original.columns[featIndex]].dtypes=='object':
       ...
       for splitVal in poss_comb:
           ...
           if (np.shape(mat0)[0] < tolN) or (np.shape(mat1)[0] <tolN):
               continue
           newS = abs(leafType(mat0)-leafType(mat1))
...
model3_1 = createTree(df, df2, 10, corLeaf, ops=(0.01,10))
